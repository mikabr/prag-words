...
What role does pragmatic reasoning play in word learning? As a child interacts with the world and acquires their native language, they need to figure out both what words mean in general (semantics) and what they mean in a specific context (pragmatics). A number of models have explored how a learner might accomplish each of these tasks on their own: cross-situational word learning models build a lexicon from observed data, while pragmatic reasoning models use an assumed lexicon to make inferences about intended meanings in context. To build a learner that can solve both problems jointly, a model would need to encode uncertainty over the lexicon and combine the mechanisms of word learning and pragmatic reasoning to both learn a lexicon and infer contextual meaning.

One model that attempts this combination is [Smith 2013](https://papers.nips.cc/paper/4929-learning-and-using-language-via-recursive-pragmatic-reasoning-about-other-agents.pdf). In this framework, speakers and listeners are modeled as not knowing a lexicon, but assuming that a conventional lexicon exists and that others know this lexicon. These assumptions avoid the exponential complexity of propagating uncertainty over lexica through levels of reasoning about others. Simulations using this model capture phenomena from word learning, pragmatic reasoning, and their interaction.

This project is a re-implementation of the Smith 2013 model and an attempt to replicate its simulation results, some of which turn out very similar to the original results, while others are not -- it remains to be seen whether the source of these discrepancies lies in the mathematical structure of the models, their implementational specifics, the setup of the simulations, or something else.
...
$$ L_0(w|o,\mathcal{L}) \propto \mathcal{L}(o,w) \cdot \text{prior}(o)$$
...
$$ L_n(o|w,\mathcal{L}) \propto S_{n-1}(w|o,\mathcal{L}) \cdot \text{prior}(o) $$
...
- sample a lexicon from the importance distribution, a Dirichlet distribution over objects for each word (`lexicon_sample`)
- compute the probability density of the sampled lexicon under the importance distribution (`lexicon_importance`)
...
Some simulation helper functions.
...
# given a literal lexicon and pragmatic agent (listener_n() or speaker_n()), compute that agent's conditional probability of each word given each object (or vice versa)
...
horn_result <- function(agent_type) {
  horn_lexica %>%
    map(~agent_posterior(.x, agent_type(.x)) %>%
          select(-true)) %>%
...
With repeated exposure to the same novel word and object pairing, children don't just make on on the fly inference as above, but actually add the association to their lexicon. Similarly, the model infers a lexicon given data in the form of occurrences of word `N` with object `B`.

```{r}
conc_ratio <- 20
mutex_lexicon_concentrations <- mutex_lexicon %>%
  mutate(conc = true * (conc_ratio - 1) + 1) %>%
  select(o, w, conc) %>%
  spread(o, conc) %>%
  ungroup() %>%
...
After `num_examples` training examples, the listener's lexicon has shifted substantially from a prior in which `N` is equally likely to refer to `A` and `B` to a posterior in which `N` is more likely to refer to `B`.
...
Without any agreed upon word-object mappings, pragmatically reasoning agents can converge on a novel conventional lexicon.
...
dialog_lexica <- function(turns) {
  
  speaker_lexica <- 1:length(turns) %>%
    map_df(~turns[[.x]]$lexica$speaker %>% mutate(i = .x, role = "speaker"))
  
  listener_lexica <- 1:length(turns) %>%
    map_df(~turns[[.x]]$lexica$listener %>% mutate(i = .x, role = "listener"))
...
plot_dialog_understanding <- function(dialog_lexica, params) {
  understands <- data_frame(
    i = unique(dialog_lexica$i),
    p = p_understands(dialog_lexica, params)
  )
  ggplot(understands, aes(x = i, y = p)) +
    geom_line() +
    ylim(c(0, 1)) +
...
Simulation with two words and two objects.
...
dialog_lexica_2 <- dialog_lexica(dialog_2)
```
...
Simulation with three words and three objects.
...
Simulation with two words and two objects, with a common and a rare object and an expensive and a cheap word.
...
dialog_lexica_horn <- dialog_lexica(dialog_horn)
```
